const AVAILABLE_TOPICS = ['fisiologia', 'cardiologia', 'citologia', 'sis-nervoso', 'histologia'];
...
let hintCountdownTimer: ReturnType<typeof setTimeout> | undefined; // Specific timer for hints
...
function getDifficultyFromStorage(): string {
...
async function selectTopic(topicName: string) {
    ...
    try {
        ...
        // Build filename with difficulty suffix only when applicable
        const fileName = `${topicName.toLowerCase()}${shouldLoadBaseFile ? '' : `_${difficulty}`}.json`;
        ...
        if (!response.ok) {
            // If specific difficulty file doesn't exist, try loading the base file
            console.warn(`File not found: ${fileUrl}, trying base file...`);
            const baseResponse = await fetch(`./data/${topicName.toLowerCase()}.json`);
            if (!baseResponse.ok) {
                throw new Error(`HTTP error! status: ${baseResponse.status}`);
            }
            currentTopicQuestions = await baseResponse.json();
        } else {
        ...
        if (rouletteDisplayElement) rouletteDisplayElement.textContent = `${formatTopicLabel(topicName)} - ${displayDifficulty.charAt(0).toUpperCase() + displayDifficulty.slice(1)}`;
        if (spinButton) {
...
function pickRandomQuestion() {
    ...
    if (unansweredQuestions.length === 0) {
        ...
        if (rouletteDisplayElement) rouletteDisplayElement.textContent = `Todas as perguntas de ${topicLabel} foram respondidas! Clique para reiniciar.`;
        if (spinButton) {
...
async function enviarDados(question: string, answer: string, rightAnswer: string, alternativeAnswers: string[]) {
...
async function checkAnswer() {
    ...
    try {
        const correcao_gemini = await enviarDados(
